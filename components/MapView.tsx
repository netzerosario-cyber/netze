...
// Pills NO usan translateY → no se mueven jamás
// ============================================================
...
// ── CSS: SIN translateY para evitar movimiento ───────────────
const CSS = `
.np{cursor:pointer;display:inline-flex;flex-direction:column;align-items:center;pointer-events:auto}
.np-p{display:flex;align-items:center;gap:5px;padding:5px 11px 5px 8px;border-radius:999px;background:#0042cd;border:1.5px solid #0042cd;box-shadow:0 2px 8px rgba(0,0,0,.15);transition:box-shadow .2s,background .2s;white-space:nowrap}
.np-p:hover{background:#0062fa;border-color:#0062fa;box-shadow:0 6px 20px rgba(0,98,250,.35)}
...
.np-a{width:0;height:0;border-left:6px solid transparent;border-right:6px solid transparent;border-top:7px solid #0042cd;transition:border-top-color .2s}
.np-p:hover+.np-a{border-top-color:#0062fa}
...
function popupHTML(prop: Property): string {
  ...
  return `<div style="width:280px;font-family:system-ui,sans-serif;margin:-10px -10px 0">
...
${photos.length>1?`
<button onclick="(function(){var c=document.getElementById('${uid}');var i=Math.max(0,(parseInt(c.dataset.i)||0)-1);c.dataset.i=i;c.style.transform='translateX(-'+(i*${100/photos.length})+'%)'})()" style="position:absolute;left:6px;top:50%;transform:translateY(-50%);width:28px;height:28px;border-radius:50%;background:rgba(255,255,255,.9);border:none;cursor:pointer;font-size:14px;font-weight:bold;color:#374151;z-index:2;box-shadow:0 1px 4px rgba(0,0,0,.15)">‹</button>
<button onclick="(function(){var c=document.getElementById('${uid}');var i=Math.min(${photos.length-1},(parseInt(c.dataset.i)||0)+1);c.dataset.i=i;c.style.transform='translateX(-'+(i*${100/photos.length})+'%)'})()" style="position:absolute;right:6px;top:50%;transform:translateY(-50%);width:28px;height:28px;border-radius:50%;background:rgba(255,255,255,.9);border:none;cursor:pointer;font-size:14px;font-weight:bold;color:#374151;z-index:2;box-shadow:0 1px 4px rgba(0,0,0,.15)">›</button>
`:''}
</div>
...
<a href="https://wa.me/${wa}?text=${wt}" target="_blank" onclick="fetch('/api/track',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({property_id:'${prop.id}',event_type:'whatsapp_click',property_title:'${sT}',property_address:'${sA}'})})" style="display:flex;align-items:center;justify-content:center;gap:6px;background:#25D366;color:#fff;font-size:12px;font-weight:700;padding:10px;border-radius:10px;text-decoration:none;box-shadow:0 2px 8px rgba(37,211,102,.3)"><svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>Consultar por WhatsApp</a>
...
export default function MapView({ properties, selectedId, isDark = false, onBoundsChange, onPropertySelect, flyToLocation }: Props) {
  ...
  useEffect(() => {
    ...
    const map = new mapboxgl.Map({ container: cRef.current, style: isDark ? 'mapbox://styles/mapbox/dark-v11' : 'mapbox://styles/mapbox/light-v11', center: CENTER, zoom: ZOOM, attributionControl: false });
    ...
    map.on('data', (e) => { if (e.sourceId === SRC && e.isSourceLoaded) sync(map); });
    mRef.current = map;
