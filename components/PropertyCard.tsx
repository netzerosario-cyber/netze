...
function buildWhatsAppUrl(property: Property, price: string): string {
  const number = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER ?? '5493415937173';
  ...
  return (
    ...
    >
      ...
      <div className="relative overflow-hidden bg-gray-100 dark:bg-[#21262d] aspect-[16/10] shrink-0">
        ...
        <div className="absolute top-2 right-2 md:top-3 md:right-3 flex items-center gap-1">
          ...
          <button
            ...
            className="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-white/95 dark:bg-[#161b22]/95 backdrop-blur-sm rounded-full shadow-sm transition-transform active:scale-90 text-gray-500 hover:text-[#0041CE] dark:text-gray-400 dark:hover:text-[#0061FB]"
          >
          ...
          <button
            ...
            className="w-7 h-7 md:w-8 md:h-8 flex items-center justify-center bg-white/95 dark:bg-[#161b22]/95 backdrop-blur-sm rounded-full shadow-sm transition-transform active:scale-90"
          >
            <svg
      ...
      <div className="px-2.5 pt-2 pb-2.5 md:p-4 flex flex-col flex-1">
        ...
        {/* Dirección */}
        <p className="text-[12px] md:text-sm text-gray-500 dark:text-gray-400 truncate mt-0.5 mb-1.5 md:mb-3">
          {property.address}
        </p>

        {/* Atributos — línea compacta */}
        <div className="flex items-center gap-2 md:gap-3.5 text-[11px] md:text-[12px] text-gray-400 dark:text-gray-500 mb-2 md:mb-4">
          {property.rooms != null && property.rooms > 0 && (
            <span className="flex items-center gap-0.5">
              <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8} className="md:w-3.5 md:h-3.5">
        ...
        <div className="flex items-center gap-1.5 mt-auto min-w-0">
          ...
          <a
            ...
            className="w-9 h-9 xl:w-auto xl:h-auto xl:flex-1 flex items-center justify-center gap-1.5 xl:py-2.5 rounded-xl bg-[#25D366] hover:bg-[#1ebe5d] active:scale-[0.98] text-white text-[12px] font-semibold transition-all shadow-sm shrink-0"
          >
          ...
          {/* Ver detalle — botón principal */}
          <Link
            href={`/propiedad/${property.id}`}
            onClick={(e) => e.stopPropagation()}
            className="flex-1 flex items-center justify-center h-9 md:py-2.5 rounded-xl border border-gray-200 dark:border-[#30363d] text-gray-600 dark:text-gray-300 text-[12px] md:text-[13px] font-semibold hover:border-[#0041CE] hover:bg-[#0041CE] hover:text-white transition-all duration-150 active:scale-[0.98]"
          >
            Ver detalle
          </Link>

          {/* Comparar — solo en desktop */}
          <button
            onClick={(e) => { e.stopPropagation(); toggleComparar(property); }}
            title={isComp ? 'Quitar de comparación' : 'Agregar a comparación'}
            className={`hidden md:flex w-10 items-center justify-center rounded-xl border text-[13px] py-2.5 transition active:scale-[0.98] ${
              isComp
