    ...
    return (
      <div className="max-w-5xl">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[1,2,3].map(i => (
            <div key={i} className="h-28 rounded-2xl bg-gray-100 dark:bg-[#161b22] animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-gray-100 dark:border-[#30363d] p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-3xl font-bold text-gray-900 dark:text-white">{data.totals.views}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Vistas totales</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-gray-100 dark:border-[#30363d] p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-3xl font-bold text-gray-900 dark:text-white">{data.totals.whatsapp_clicks}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Consultas WhatsApp</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-gray-100 dark:border-[#30363d] p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-3xl font-bold text-gray-900 dark:text-white">{data.totals.map_clicks}</p>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Clicks en mapa</p>
          </div>
        </div>
      </div>
      ...
      {data.dailyTrend.length > 0 && (
        <div className="bg-white dark:bg-[#161b22] rounded-2xl border border-gray-100 dark:border-[#30363d] p-5 mb-8">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Últimos 7 días</h3>
        ...
        <div className="flex flex-col gap-2">
          {items.map((item, i) => (
            <Link
              ...
              target="_blank"
              className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-gray-50 dark:hover:bg-[#1c2128] transition-colors"
            >
              <span className="w-6 h-6 rounded-full bg-gray-100 dark:bg-[#30363d] flex items-center justify-center text-[10px] font-bold text-gray-500 dark:text-gray-400 shrink-0">
                {i + 1}
              </span>
              ...
              <span className="text-sm font-bold text-[#0041CE] dark:text-[#0061FB] shrink-0">{item.count}</span>
            </Link>
          ))}
        </div>
      )}
